...
### 6. Задачи
#### 6.1 Искусственные данные по средней продолжительности сна
[В данных по продолжительности сна](https://goo.gl/T22KZP) две переменных: код испытуемого и среднее время сна. Попробуйте сделать следующий график:
```{r}
read.csv("https://goo.gl/T22KZP") %>% 
  mutate(sex = gsub("[0-9]|_", "", code)) %>% 
  ggplot(aes(sleep_hours, fill = sex))+
  geom_density(alpha= 0.4)+
  theme_bw()+
  labs(title = "Зависимость времени сна от пола",
       subtitle = "искусственные данные",
       y = "плотность",
       x = "среднее время сна")
